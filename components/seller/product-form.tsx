  ...
  const handleSubmit = async (e: React.FormEvent, isDraft = false) => {
    ...
    try {
      if (mode === "create") {
        ...
        const newFiles: { file: File; sort_order: number }[] = [];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-white/10">
        {isSubmitting ? (
          <span className="w-full sm:w-auto px-5 py-2.5 text-xs font-semibold text-[#8f7d8c] bg-white/[0.02] border border-white/5 rounded-xl text-center cursor-not-allowed pointer-events-none select-none">
            Cancel
          </span>
        ) : (
          <Link
